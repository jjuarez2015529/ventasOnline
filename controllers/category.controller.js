        ...
        if(err){
            ...
            category.save((err,categorySaved)=>{
                if(err){
                    ...
                    console.log("Error al crear la categoría Default");
                }
            })
        }
    })
    ...
    if(params.name){
        Category.findOne({name: params.name},(err,categoryFind)=>{
            if(err){
                ...
                return res.send({message: "La categoría ya existe"});
            }else{
                var category = new Category();
        ...
        return res.status(403).send({message: "Ingrese los datos solicitados"})
    }
}
                ...
                if(err){
                    ...
                    Category.findOne({_id : categoryId},(err,categoryFind)=>{
                        if(err){
                            ...
                            return res.status(403).send({message: "ID de categoría no existe o ya fue eliminada"});
                        }
                    })
                }else{
...
function getCategories(req,res){
    Category.find({}).populate("products").exec((err,categories)=>{
        if(err){
            ...
            return res.status(403).send({message: "No hay existen datos"});
        }
    })
}
