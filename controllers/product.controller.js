    ...
    if(params.name && params.price && params.stock){
        Category.findById(categoryId,(err,categoryFind)=>{
            if(err){
                            ...
                            if(err){
                                ...
                                Category.findByIdAndUpdate(categoryId,{$push:{products:productSaved._id}},{new: true},(err,categoryUpdated)=>{
                                    if(err){
                                        ...
                                        return res.send({message: "Producto agregado a la categoría",categoryUpdated});
                                    }else{
                ...
                return res.status(403).send({message: "La categoria no existe"});
            }
        })
    }else{
        return res.status(403).send({message: "Ingrese todos todos datos solicitados"});
    }
}
...
function removeProduct(req,res){
    ...
    Category.findOneAndUpdate({_id:categoryId,products:productId},{$pull:{products:productId}},{new:true},(err,categoryUpdated)=>{
        if(err){
            ...
            return res.status(404).send({message: "El producto no existe o ya fue eliminado"});
        }
    })
}
