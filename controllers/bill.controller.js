            ...
            if(cartFind.products != ""){
                    ...
                    Product.findOne({_id:element},(err,productFind)=>{
                        if(err){
                            ...
                            if(stockP<cantidad[i]){
                                i++;
                                return res.send({message: "Cantidad de carrito no es válida"});
                            }else{
                                i++;
                            }
                        }else{
                ...
                bill.save((err,billSaved)=>{
                    if(err){
                        ...
                        return res.status(404).send({message: "No se creo la factura"});
                    }
                })
            }else{
    ...
    if(req.user.role == "ROLE_ADMIN"){
        Bill.find({}).exec((err,bills)=>{
            if(err){
                ...
                return res.status(403).send({message: "No hay existen facturas por mostrar"});
            }
        })
    }else{
...
function getProductsBill(req,res){
    ...
    Bill.findById({_id:billId}).populate("products").exec((err,billFind)=>{
        if(err){
            ...
            return res.send({message: "Los productos de la factura: ",productos});
        }else{
