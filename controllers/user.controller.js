                ...
                if(err){
                    ...
                    user.save((err,userSaved)=>{
                        if(err){
                            ...
                            console.log("Usuario ADMIN ha sido creado");
                        }else{
                            console.log("No se pudo crear el usuario ADMIN");
                        }
                    })
                }else{
...
function login(req,res){
    ...
    if(params.username && params.password){
        ...
        return res.status(403).send({message: "Ingrese todos los datos solicitados"});
    }
}
    ...
    if(params.name && params.username && params.password && params.email){
        User.findOne({username: params.username},(err,userFind)=>{
            if(err){
                ...
                return res.send({message: "El nombre de usuario se ha utilizado"});
            }else{
                    ...
                    if(userFinded.role == "ROLE_CLIENTE"){
                        User.findByIdAndUpdate(userId,update,{new:true},(err,userUpdated)=>{
                            if(err){
                                ...
                                return res.send({message: "El usuario se actualizado",userUpdated});
                            }else{
    ...
    if(userId == req.user.sub || req.user.role == "ROLE_ADMIN"){
        User.findById(userId,(err,userFind)=>{
            if(err){
                ...
                if(userFind.role == "ROLE_CLIENTE"){
                    ...
                    User.findByIdAndRemove(userId,(err,userRemoved)=>{
                        if(err){
                            ...
                            return res.send({message: "El usuario se ha removido"});
                        }else{
                            return res.status(403).send({message: "El usuario no existe o ya fue eliminado"});
                        }
                    })
                }else{
                ...
                return res.status(403).send({message: "El ID de usuario no existe o ya fue eliminado"});
            }
        })
    }else{
